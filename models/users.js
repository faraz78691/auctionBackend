...
module.exports = {
  ...
  getUserNamebyId: async (id) => {
    return db.query(`select user_name AS name, profile_image from  users where  id=${id}`);
  },

  getAllMessageByUserId: async (id) => {
    return db.query(`SELECT users.user_name AS full_name, users.profile_image, users.online_status, tbl_messages.* FROM tbl_messages LEFT JOIN users ON users.id = tbl_messages.user_id WHERE tbl_messages.user_id = ${id} ORDER BY tbl_messages.created_at;`);
  },
